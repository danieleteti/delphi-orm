...
type
  ...
  protected
    ...
    procedure SetFireDACParameterValue(AFieldType: string; AStatement: TFDQuery;
...
begin
  ...
  begin
    ...
    pk_field_name := AMappingTable.Fields[pk_idx].FieldName;
    SQL := 'SELECT ' + GetSelectFieldsList(AMappingTable.Fields, true) + ' FROM ' +
      AMappingTable.TableName + ' WHERE [' + AMappingRelationField.FieldName + '] = :' + pk_field_name;
  end;
  GetLogger.Debug('PREPARING: ' + SQL);
  Result := FD.NewQuery;
  Result.SQL.Text := SQL;
...
procedure TFireDACBaseAdapter.LoadObjectFromFireDACReader(AObject: TObject; ARttiType: TRttiType;
  AReader: TFDQuery; AFieldsMapping: TMappingFieldList);
var
  field       : TMappingField;
  v           : TValue;
  S           : string;
