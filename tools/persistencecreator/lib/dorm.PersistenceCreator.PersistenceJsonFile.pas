...
type
  ...
  strict private
    ...
    function PersistenceJSONObjectExist(ASection: string; APersistenceJSONObjectName: string = ''): Boolean;
...
var persistenceReader: TStreamReader;
    s: string;
begin
  inherited;
  FPersistenceJsonFileName := APersistenceFileName;

  if FPersistenceJsonFileName='' then raise Exception.Create('Persistence file not found');
...
var PersClasses: TPersistenceClass;
...
begin
  ...
  if ArrayClasses.Length = 0 then Exception.Create('persistent_classes non present or not valid');
...
begin
  ...
  begin
    ...
    begin
      raise Exception.Create('Cannot parse persistence file. Missing persistence JSON object '+APersistenceJSONObjectName);
    end;
  end else
  begin
    if FPersistenceJsonFile.O[ASection] = nil then
    begin
      raise Exception.Create('Cannot parse persistence file. Missing persistence JSON section '+APersistenceJSONObjectName);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    JSON['persistence'].O[FEnvironments[i].EnvironmentName].S['database_adapter'] := FEnvironments[i].DatabaseAdapter;
    JSON['persistence'].O[FEnvironments[i].EnvironmentName].S['database_connection_string'] := FEnvironments[i].DatabaseConnectionString;
    JSON['persistence'].O[FEnvironments[i].EnvironmentName].S['keys_generator'] := FEnvironments[i].KeysGenerator;
    JSON['persistence'].O[FEnvironments[i].EnvironmentName].S['key_type'] := FEnvironments[i].KeyType;
    JSON['persistence'].O[FEnvironments[i].EnvironmentName].S['null_key_value'] := FEnvironments[i].NullKeyValue;

    for x := 0 to FEnvironments[i].CustomAdapterParameter.Count-1 do
    begin
      JSON['persistence'].O[FEnvironments[i].EnvironmentName].S[FEnvironments[i].CustomAdapterParameter.Names[x]] := FEnvironments[i].CustomAdapterParameter.ValueFromIndex[x];
    end;
  end;
  ...
  begin
    PersistentClasses.AsArray.S[i] := FPersistenceClasses[i].PersistenceClassName;
  end;
...
var
  ...
  i: Integer;
begin
  AEnv.DatabaseAdapter := FPersistenceJsonFile.O['persistence'].O[AEnvironments].s['database_adapter'];
  AEnv.DatabaseConnectionString := FPersistenceJsonFile.O['persistence'].O[AEnvironments].s['database_connection_string'];
  AEnv.KeysGenerator := FPersistenceJsonFile.O['persistence'].O[AEnvironments].s['keys_generator'];
  AEnv.KeyType := FPersistenceJsonFile.O['persistence'].O[AEnvironments].s['key_type'];
  AEnv.NullKeyValue := FPersistenceJsonFile.O['persistence'].O[AEnvironments].s['null_key_value'];

  Obj := FPersistenceJsonFile.O['persistence'].O[AEnvironments].AsObject.GetNames;
  Obj1 := FPersistenceJsonFile.O['persistence'].O[AEnvironments].AsObject.GetValues;
  ...
  begin
    if (ArrNames.S[i]<>'database_adapter') and
       ...
       (ArrNames.S[i]<>'keys_generator') and
       (ArrNames.S[i]<>'key_type') and
...
var Cfg: TPersistenceConfig;
begin
  inherited;
  Cfg := TPersistenceConfig.Create;
  Cfg.LoggerClassName := FPersistenceJsonFile.O['config'].s['logger_class_name'];
  FConfig := Cfg;
end;
